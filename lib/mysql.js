...
// trigger
let createTriggerLog = function() {
    let _sql = `CREATE TRIGGER user_log AFTER INSERT ON users FOR EACH ROW
				BEGIN
...
let deleteFriend = function(value) {
    let _sql = 
        `DELETE FROM friends
         WHERE (friends.uid1 = ? AND friends.uid2 = ?)
         OR (friends.uid1 = ? AND friends.uid2 = ?);`
    return query(_sql, value)
}

//likes
let insertLike = function(value) {
    //console.log(value)
    let _sql = `INSERT INTO likes SET mid = ?, uid = ?;`
    return query(_sql, value)
}

let deleteLike = function(value) {
    let _sql = `DELETE FROM likes WHERE mid = ? AND uid = ?;`
    return query(_sql, value)
}

let findLikesByMid = function(value) {
    let _sql = 
        `SELECT users.id, users.name
         FROM likes, users
         WHERE likes.mid = ?
         AND likes.uid = users.id;`
    return query(_sql, value)
}

let findLikesByMidUid = function(value) {
    let _sql = 
        `SELECT * FROM likes WHERE mid = ? AND uid = ?;`
    return query(_sql, value)
}
